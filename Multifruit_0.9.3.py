...
def save_data(fly_name, velocity_data, movement_thresholds, sound_start_time, led_on_time, objects=None, sound_positions=None):
    with open(f"{fly_name}_movement_thresholds.csv", mode='w', newline='') as file:
        ...
        writer.writerow(['Object',
                ...
                # ✅ NEW: fly position at sound onset
                'X at Sound (px)',
                'Y at Sound (px)',
                'X at Sound (mm)',
                'Y at Sound (mm)',
        ...
        for obj_id in movement_thresholds.keys():
            ...
            # ✅ NEW: position of the fly at the moment the sound started
            missing = 'No sound played' if sound_start_time is None else 'Not recorded'
            x_px, y_px, x_mm, y_mm = missing, missing, missing, missing
            detected_at_sound = ''
            if sound_positions and sound_positions.get(obj_id) is not None:
                pos_x, pos_y, was_detected = sound_positions[obj_id]
                x_px = f'{pos_x:.2f}'
                y_px = f'{pos_y:.2f}'
                x_mm = f'{pos_x * PIXELS_TO_MM:.3f}'
                y_mm = f'{pos_y * PIXELS_TO_MM:.3f}'
                detected_at_sound = 'Yes' if was_detected else 'No (chamber centre fallback)'
...
def main():
    ...
    # ✅ NEW: (x, y, was_really_detected) of each fly at the exact frame the sound starts
    sound_positions = {i: None for i in range(expected_objects)}
        ...
        if grabResult.GrabSucceeded():
            ...
            if not sound_played and elapsed_time >= MIN_TIME_BEFORE_SOUND:
                if led_on_time is None and still_objects >= STILLNESS_OBJECT_COUNT:
                    ...
                    # ✅ NEW: capture each fly's X/Y position on the frame the sound starts
                    for obj in objects:
                        if obj.id in active_chambers:
                            sound_positions[obj.id] = (
                                float(obj.center[0]),
                                float(obj.center[1]),
                                bool(obj.real_detection)
                            )
                            note = '' if obj.real_detection else '  [NOT DETECTED - chamber centre fallback]'
                            print(f"  Object {obj.id+1} position at sound onset: "
                                  f"x={obj.center[0]:.1f} px, y={obj.center[1]:.1f} px{note}")
    ...
    save_data(fly_name, velocity_data, movement_thresholds, sound_start_time, led_on_time, objects, sound_positions)
